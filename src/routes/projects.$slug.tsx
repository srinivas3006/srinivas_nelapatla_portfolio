...
import { ArrowLeft, ArrowUpRight, Calendar, Clock, User, Sparkles, CheckCircle2, Leaf } from "lucide-react";
...
export const Route = createFileRoute("/projects/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.project.title} — Srinivas Nelapatla` },
          { name: "description", content: loaderData.project.overview },
          { property: "og:title", content: loaderData.project.title },
          { property: "og:description", content: loaderData.project.overview },
      ...
      <article className="mx-auto max-w-5xl px-6 pb-24 pt-12">
        ...
        >
          <p className="text-xs uppercase tracking-[0.3em] text-primary">
            ✦ {project.cat === "web" ? "Web App" : project.cat === "data" ? "Data Project" : "Tool"}
          </p>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            ...
            <Meta icon={Clock} label="Timeline" value={project.timeline} />
          </div>
        </motion.div>
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-[1.4fr_1fr]">
          <div className="space-y-10">
            ...
            <Section title="The approach">{project.solution}</Section>
          ...
          <aside className="space-y-6">
            <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {project.stack.map((s) => (
                  <span key={s} className="rounded-full bg-secondary px-3 py-1 text-xs text-foreground/80">
                    {s}
                  </span>
                ))}
              </div>
              ...
              {others.map((o) => (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{o.desc}</p>
                  </div>
                </Link>
              ))}
            </div>
